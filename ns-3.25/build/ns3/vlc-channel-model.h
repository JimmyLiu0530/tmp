...
public:
	...
	void SetPropagationLossModel(ns3::Ptr<ns3::VlcPropagationLossModel> loss); //sets the propagation loss model for the channel

	ns3::Ptr<ns3::VlcPropagationLossModel> GetPropagationLossModel();//returns a pointer to the propagation loss model
	...
	void InstallOtherChannel(std::pair<std::string, ns3::Ptr<VlcChannel>> other);

	void AddInterference(double power, std::string chName);
	void AggregateSignal(double power, std::string txName);

	void SubstractInterference(double t);

	double GetAngleOfRadiance (Ptr<VlcMobilityModel> tx, Ptr<VlcMobilityModel> rx) const;
	double GetIncidenceAngle (Ptr<VlcMobilityModel> tx, Ptr<VlcMobilityModel> rx) const;

	void AttachPropogationLossModel(Ptr < VlcNetDevice > tx, Ptr < VlcNetDevice > rx);

	void AccumulatePacketCount();
	int GetPacketCount();

private:
	ns3::Ptr<ns3::VlcPropagationLossModel> m_loss;
	...
	//id: channel name
	std::map<std::string, ns3::Ptr<VlcChannel> > m_other_channel;
