...
public:
	...
	void AttachReceiver(std::string chName, std::string RXDevName,
			ns3::Ptr<VlcDeviceHelper> devHelper);
	//new
	void AttachTransmitter(std::string chName, Ptr<VlcTxNetDevice> tx);

	void AttachReceiver(std::string chName, Ptr<VlcRxNetDevice> rx);
	//new
	...
	std::map<std::string, ns3::Ptr<VlcChannel> > GetAllChannels();
	...
	NetDeviceContainer InstallwithInterference(Ptr<Node> a, Ptr<Node> b,
			Ptr<VlcDeviceHelper> devHelper, Ptr<VlcChannelHelper> chHelper,
			std::string txName, std::string rxName, std::string chName);

	Ptr<VlcChannel> InstallwithInterference(Ptr<Node> a, Ptr<Node> b,
			std::string chName);
