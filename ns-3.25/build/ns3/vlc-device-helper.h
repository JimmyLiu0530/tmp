...
public:
	...
	//New
	void SetTrasmitterPosition(std::string devName, Vector pos);

	void SetReceiverPosition(std::string devName, Vector pos);

	//New
	std::map<std::string, ns3::Ptr<VlcTxNetDevice> > GetAllTransmitter();

	std::map<std::string, ns3::Ptr<VlcRxNetDevice> > GetAllReceiver();

	//New
	void SetTxMobility(std::string devName, Ptr<VlcMobilityModel> mobilityModel);

	void SetRxMobility(std::string devName, Ptr<VlcMobilityModel> mobilityModel);
