...
void VlcErrorModel::SetScheme(ModScheme scheme)	// sets the value of the modulation scheme used
	...
	else if(scheme == QAM64) {
		mod_order = 4;
	}
	else if(scheme == QAM256) {
		mod_order = 5;
	}
...
double VlcErrorModel::CalculateErrorRate(void) {
	...
	if (mod_scheme == VPPM)
		...
		return CalculateSER();
				...
				mod_scheme == QAM64 || mod_scheme == QAM256 || mod_scheme == BPSK)
			return ComputeBER();
...
bool VlcErrorModel::IsCorrupt(Ptr<Packet> p) {
	...
	if (mod_scheme == PAM4 || mod_scheme == OOK || mod_scheme == PAM8 || mod_scheme == PAM16) {
		...
		// std::cout << m_ranvar->GetValue() << std::endl;
...
double VlcErrorModel::ComputeBER() {
	...
	errorRate = sqrt((3 * snr)/(2 * (pow(2, 2 * mod_order) - 1)));
	errorRate = erfc(errorRate);
	errorRate *= (pow(2, mod_order) - 1) / (mod_order * pow(2, mod_order));

	//std::cout << errorRate << std::endl;
	if(errorRate > 1.e-4) {
		std::cout << "Wrong: ";
		std::cout << errorRate << std::endl;
		throw std::logic_error("Wrong maximum BER");
	}
