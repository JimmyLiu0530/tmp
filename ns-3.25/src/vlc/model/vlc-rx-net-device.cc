...
void VlcRxNetDevice::SetRXGain() {
	NS_LOG_FUNCTION(this);
	//std::cout << this->m_angleOfIncidence << std::endl;
...
void VlcRxNetDevice::SetIncidenceAngle(double angle) {
	...
	this->SetRXGain();
	this->SetConcentrationGain();
}

ns3::VlcErrorModel::ModScheme VlcRxNetDevice::GetModulationtScheme(void) const {	// returns the modulation scheme used
	NS_LOG_FUNCTION(this);
...
void VlcRxNetDevice::ClearBuffer() {
	dataPool.clear();
}

void VlcRxNetDevice::SetResponsivity(double res) {
	m_responsivity = res;
}
double VlcRxNetDevice::GetResponsivity() {
	return m_responsivity;
}

void VlcRxNetDevice::Active() {
	IsIdle = false;
}

void VlcRxNetDevice::Idle() {
	IsIdle = true;
}

bool VlcRxNetDevice::GetDeviceStatus() {
	return !IsIdle;
}

void VlcRxNetDevice::SetTime(double seconds) {
	m_time = seconds;
}

double VlcRxNetDevice::GetTime() {
	return m_time;
}
