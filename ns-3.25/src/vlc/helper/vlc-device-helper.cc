...
// 		std::logic_error("Frame Size not present");
		...
		m_TXDevices[devName]->SetTxPowerMax(value);
	} else if (paramName == "RMax") {
		m_TXDevices[devName]->SetTXRateMax(value);
	}
...
void VlcDeviceHelper::SetTrasmitterPosition(std::string devName, Vector pos) {
	m_TXDevices[devName]->SetPosition(pos);
}

void VlcDeviceHelper::SetReceiverPosition(std::string devName, Vector pos) {
	m_RXDevices[devName]->SetPosition(pos);
}
...
void VlcDeviceHelper::SetReceiverParameter(std::string devName,
	...
	} else if (paramName == "Responsivity") {
		m_RXDevices[devName]->SetResponsivity(value);
	}
...
std::map<std::string, ns3::Ptr<VlcTxNetDevice> > VlcDeviceHelper::GetAllTransmitter() {
	return this->m_TXDevices;
}

std::map<std::string, ns3::Ptr<VlcRxNetDevice> > VlcDeviceHelper::GetAllReceiver() {
	return this->m_RXDevices;
}

void VlcDeviceHelper::SetTxMobility(std::string devName, Ptr<VlcMobilityModel> mobilityModel) {
	m_TXDevices[devName]->SetMobilityModel(mobilityModel);
}

void VlcDeviceHelper::SetRxMobility(std::string devName, Ptr<VlcMobilityModel> mobilityModel) {
	m_RXDevices[devName]->SetMobilityModel(mobilityModel);
}
