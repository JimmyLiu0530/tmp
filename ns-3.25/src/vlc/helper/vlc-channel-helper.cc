...
void VlcChannelHelper::AttachTransmitter(std::string chName, Ptr<VlcTxNetDevice> tx) {
	ns3::Ptr < VlcPropagationLossModel > loss = ns3::DynamicCast
			< VlcPropagationLossModel
			> (m_channel[chName]->GetPropagationLossModel());
...
void VlcChannelHelper::AttachReceiver(std::string chName, Ptr<VlcRxNetDevice> rx) {
	ns3::Ptr < VlcPropagationLossModel > loss = ns3::DynamicCast
			< VlcPropagationLossModel
			> (m_channel[chName]->GetPropagationLossModel());
	...
	loss->SetArea(rx->GetPhotoDetectorArea());
}
...
std::map<std::string, ns3::Ptr<VlcChannel> > VlcChannelHelper::GetAllChannels() {
	return m_channel;
}
...
//including interference setting
NetDeviceContainer VlcChannelHelper::InstallwithInterference(Ptr<Node> a, Ptr<Node> b,
		Ptr<VlcDeviceHelper> devHelper, Ptr<VlcChannelHelper> chHelper,
		std::string txName, std::string rxName, std::string chName) {

	NetDeviceContainer container;
	container = chHelper->Install(a, b, devHelper, chHelper, txName,
			rxName, chName);

	ns3::Ptr<VlcChannel> this_channel = m_channel[chName];
	for(std::map<std::string, ns3::Ptr<VlcChannel> > ::iterator it = m_channel.begin();
			it != m_channel.end(); ++it) {
		if(it != m_channel.end()) {
			this_channel->InstallOtherChannel(std::pair<std::string, ns3::Ptr<VlcChannel>>
					(it->first, it->second));
			it->second->InstallOtherChannel(std::pair<std::string, ns3::Ptr<VlcChannel>>
					(chName, this_channel));
		}
	}
...
Ptr<VlcChannel> VlcChannelHelper::InstallwithInterference(Ptr<Node> a, Ptr<Node> b,
		std::string chName) {

	Ptr<VlcChannel> channel;
	this->Install(chName, a, b);
	channel = this->GetChannel(chName);

	ns3::Ptr<VlcChannel> this_channel = m_channel[chName];
	for(std::map<std::string, ns3::Ptr<VlcChannel> > ::iterator it = m_channel.begin();
			it != m_channel.end(); ++it) {
		if(it != m_channel.end()) {
			//add other established channels to this object to calculate interference or aggregated signals
			this_channel->InstallOtherChannel(std::pair<std::string, ns3::Ptr<VlcChannel>>
					(it->first, it->second));
			it->second->InstallOtherChannel(std::pair<std::string, ns3::Ptr<VlcChannel>>
					(chName, this_channel));
