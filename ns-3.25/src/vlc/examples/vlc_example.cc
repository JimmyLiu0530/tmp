...
int main(int argc, char *argv[]) {
	...
	for (double dist = 0.1; dist < 2; dist += 0.1) {
		...
		devHelperVPPM.SetTrasmitterParameter("THE_TRANSMITTER1", "pMax", 1);//SETTING BIAS VOLTAGE MOVES THE SIGNAL VALUES BY THE AMOUNT OF BIASING
		...
		************************************************************************************/
		...
		devHelperVPPM.SetTrasmitterParameter("THE_TRANSMITTER2", "pMax", 1);//SETTING BIAS VOLTAGE MOVES THE SIGNAL VALUES BY THE AMOUNT OF BIASING
		...
		*************************************************************************************/
		...
		Ptr < VlcRxNetDevice > rxHandle2 = devHelperVPPM.GetReceiver(
				"THE_RECEIVER2");
		double goodput2 = rxHandle2->ComputeGoodPut();
	       std::cout<< "Good Packet Received Size is  "<< goodput2<< std::endl;
		goodput2 *= 8;
		goodput2 /= theTime.back();
		goodput2 /= 1024;
		std::cout << "Simulation time is "<< theTime.back()<<std::endl;
		std::cout << "goodput value is " << goodput2 << std::endl;
